...
ln = 0
Po = .00064 # to be changed, initial amplitude.
alpha = 0
alph = alpha/4.343
gamma = 0.003
to = 125e-12 # to be changed.
C = -2 # to be revised.
b2 = -20e-27
Ld = (to**2)/np.absolute(b2) # check it.
pi = 3.1415926535
Ao = np.sqrt(Po)
...
uaux = Ao*np.exp(-((1+1j*(-C))/2.0)*(tau/to)**2) # to be changed.
...
for ii in np.arange(0.1, 1.51, 0.1):
    ...
    for jj in np.arange(h, z+1, h):
        spectrum = spectrum*np.exp(-alph*(h/2.0)+1j*b2/2.0*(np.power(w, 2))*(h/2.0)) # to be changed.
